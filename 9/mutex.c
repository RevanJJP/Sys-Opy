...
{
    if (argc < 2) ERROR(0, "Too few arguments!");
   ...
   buffer = malloc(buff_size * sizeof(char*));
    ...
    for (i = 0; i<prs_s; i++) if (pthread_create(&prods[i], NULL, &producer, NULL)) ERROR(0, "producer thread");
    for (i = 0; i<cons_s; i++) if (pthread_create(&cons[i], NULL, &consumer, NULL)) ERROR(0, "Consumer thread");

    if (nk) alarm(nk);
    for (i = 0; i<prs_s; i++) if (pthread_join(prods[i], NULL)) ERROR(0, "Thread prod join");
    while (1)
    {
        pthread_mutex_lock(&count_mutex);
        if (count == 0) break;
...
{
    FILE* conf;
    if ((conf = fopen(filepath, "r")) < 0) ERROR(0, "Conf file open");
    char buff[1024];
    ...
    buff_size = atoi(p);
    p = strtok (NULL, " ");
    if ((buff_src = fopen(p, "r")) == NULL) ERROR(0, "Source file");
    p = strtok (NULL, " ");
    l = atoi(p);
    p = strtok (NULL, " ");
    ...
    p = strtok (NULL, " ");
    nk = atoi(p);
    if (prs_s <= 0 || cons_s <= 0 || l < 0) ERROR(0, "Incorrecr conf");
